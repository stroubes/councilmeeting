...
let ReportsService = class ReportsService {
    ...
    governanceService;
    constructor(docxParserService, sharePointDocxService, agendasService, reportsRepository, auditService, templatesService, notificationsService, governanceService) {
        this.docxParserService = docxParserService;
        this.sharePointDocxService = sharePointDocxService;
    ...
    async create(dto, user) {
        ...
        const defaultSections = template
            ...
            : [];
        const executiveSummary = dto.executiveSummary ??
    ...
    async importDocx(dto, user) {
        ...
        if (!contentBase64) {
            throw new common_1.BadRequestException('Provide contentBase64 or SharePoint drive/item identifiers for DOCX import.');
        }
    ...
    async update(id, dto, user) {
        ...
        if (report.workflowStatus === 'PUBLISHED') {
            throw new common_1.BadRequestException('Published report cannot be edited');
        }
        if (dto.templateId) {
    ...
    async submitForDirector(id, user, comments) {
        const report = await this.reportsRepository.getById(id);
    ...
    async approveDirector(id, user, comments) {
        this.ensureApproverRole(user, [roles_constants_1.SYSTEM_ROLES.DIRECTOR, roles_constants_1.SYSTEM_ROLES.ADMIN]);
        const report = await this.reportsRepository.getById(id);
    ...
    async approveCao(id, user, comments) {
        this.ensureApproverRole(user, [roles_constants_1.SYSTEM_ROLES.CAO, roles_constants_1.SYSTEM_ROLES.ADMIN]);
        const report = await this.reportsRepository.getById(id);
    ...
    async reject(id, user, stage, comments) {
        this.ensureApproverRole(user, [roles_constants_1.SYSTEM_ROLES.DIRECTOR, roles_constants_1.SYSTEM_ROLES.CAO, roles_constants_1.SYSTEM_ROLES.ADMIN]);
        const report = await this.reportsRepository.getById(id);
    ...
    async resubmit(id, user, comments) {
        const report = await this.reportsRepository.getById(id);
    ...
    async publish(id, user) {
        if (!user.permissions.includes(permissions_constants_1.PERMISSIONS.PUBLIC_PUBLISH) && !user.roles.includes(roles_constants_1.SYSTEM_ROLES.ADMIN)) {
            throw new common_1.ForbiddenException('Publish permission is required');
        }
        const report = await this.reportsRepository.getById(id);
    ...
    async addAttachment(reportId, dto, user) {
        ...
        if (dto.contentBase64?.trim()) {
            if (!dto.sharePointDriveId) {
                throw new common_1.BadRequestException('sharePointDriveId is required when uploading a file attachment.');
            }
            ...
            throw new common_1.BadRequestException('Provide contentBase64 with sharePointDriveId, or provide existing sharePointDriveId and sharePointItemId.');
        }
    ...
    async removeAttachment(reportId, attachmentId, user) {
        await this.reportsRepository.getById(reportId);
    ...
    ensureStatus(current, allowed) {
        if (!allowed.includes(current)) {
            throw new common_1.BadRequestException(`Invalid report workflow transition from ${current}. Allowed: ${allowed.join(', ')}`);
        }
    }
    ensureApproverRole(user, allowedRoles) {
        if (!allowedRoles.some((role) => user.roles.includes(role))) {
            throw new common_1.ForbiddenException('Approver role is required');
        }
    }
    async ensureAgendaItemExists(agendaItemId) {
        const exists = await this.agendasService.hasAgendaItem(agendaItemId);
        if (!exists) {
            throw new common_1.BadRequestException('Agenda item does not exist for this report');
        }
    }
    async ensureTemplateType(templateId, expectedType) {
        const template = await this.templatesService.getById(templateId);
        if (template.type !== expectedType) {
            throw new common_1.BadRequestException(`Template ${templateId} must be of type ${expectedType}`);
        }
        return template;
    }
    async getSubmissionIssues(report) {
        const issues = [];
...
exports.ReportsService = ReportsService = __decorate([
    ...
    __metadata("design:paramtypes", [docx_parser_service_1.DocxParserService,
        sharepoint_docx_service_1.SharePointDocxService,
        agendas_service_1.AgendasService,
        reports_repository_1.ReportsRepository,
